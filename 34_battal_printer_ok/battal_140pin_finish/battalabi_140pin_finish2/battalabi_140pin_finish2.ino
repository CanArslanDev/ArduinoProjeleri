...
#define PEAK_FALL2 6  // Rate of peak falling dot
#define PEAK_FALL3 14  // Rate of peak falling dot
...
byte
  ...
  dotCount  = 0,      // Frame counter for delaying dot-falling speed
    dotCount2  = 0,      // Frame counter for delaying dot-falling speed
      dotCount3  = 0,      // Frame counter for delaying dot-falling speed
...
void loop () {  
  if(x<1){
 CylonBounce(0xff, 0x00, 0x00, 8, 10, 50);
  }
    if(x<1){
 FadeInOut(0xff, 0x00, 0x00); // red
  }
    if(x<1){
  CylonBounce(0, 0, 255, 8, 10, 50);
  }
    if(x<1){
 FadeInOut(0, 0, 255); // red
  }
    if(x<1){
   CylonBounce(0, 250, 0, 8, 10, 50);
  }
    if(x<1){
 FadeInOut(0, 255, 0); // red
  }
    if(x<1){
    CylonBounce(200, 50, 255, 8, 10, 50);
  }
    if(x<1){
 FadeInOut(200, 50, 255); // red
  }
    if(x<1){
     CylonBounce(255, 255, 255, 8, 10, 50);
  }
    if(x<1){
...
if (butonsayac % 2 == 0) {  //eğer butona 2 kes basılmışsa, aktif et 0 a eşitle ve 
  ...
  if(height < 0L)       height = 0;      // Clip output
    ...
    height2 = TOP * (lvl - minLvlAvg) / (long)(maxLvlAvg - minLvlAvg);

  if(height2 < 0L)       height2 = 0;      // Clip output
  else if(height2 > TOP) height2 = TOP;

    height3 = TOP * (lvl - minLvlAvg) / (long)(maxLvlAvg - minLvlAvg);

  if(height3 < 0L)       height3 = 0;      // Clip output
  else if(height3 > TOP) height3 = TOP;
  if(height > peak)     peak   = height;// Keep 'peak' dot at top
  if(height2 > peak2)     peak2   = height2;// Keep 'peak' dot at top
  if(height3 > peak3)     peak3   = height3;// Keep 'peak' dot at top
  ...
  for(i=0; i<N_PIXELS; i++) {
    if(i >= height2)               strip.setPixelColor(i,   0,   0, 0);
    else strip.setPixelColor(i,Wheel(map(i,0,strip.numPixels()-1,30,150)));

  }
    for(i=0; i<N_PIXELS; i++) {
    if(i >= height3)               strip.setPixelColor(i,   0,   0, 0);
    else strip.setPixelColor(i,Wheel(map(i,0,strip.numPixels()-1,30,150)));
...
// Every few frames, make the peak pixel drop by 1:
  ...
  if(peak2 > 0 && peak2 <= N_PIXELS-1) strip.setPixelColor(peak2,Wheel(map(peak2,0,strip.numPixels()-1,30,150)));
...
// Every few frames, make the peak pixel drop by 1:
    ...
    if(++dotCount2 >= PEAK_FALL2) { //fall rate
      ...
      if(peak3 > 0 && peak3 <= N_PIXELS-1) strip.setPixelColor(peak3,Wheel(map(peak3,0,strip.numPixels()-1,30,150)));

   strip.show(); // Update strip

// Every few frames, make the peak pixel drop by 1:

    if(++dotCount3 >= PEAK_FALL3) { //fall rate
